...
const PersonDetailPage = props => {
	...
	const toggleState = (editMode, permission) => {
	...
	useState(() => {
		setIsLoading(true);
		db.collection('profiles').where("id", "==", props.match.params.id)
			.get()
			.then((querySnapshot) => {
	...
	const { hasProfile, title, displayName, headshotSrc, description, profileVisible, facebook, twitter, linkedin, github, portfolio } = person;
